...
volatile int32_t global_value;
volatile int32_t global_return;
...
int local_sram_read_write() {
    // API call to set the global tile ID values
    bsg_set_tile_x_y();
    int num_tiles = bsg_num_tiles;
    int tile_id = bsg_x_y_to_id(bsg_x, bsg_y);

    // Each tile adds their ID to the value (doubling it)
    global_return = global_value + tile_id;
